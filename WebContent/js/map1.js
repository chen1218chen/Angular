define(
		...
		function(jquery, BMap) {
			...
			map.createMap = function() {
				 var map = new BMap.Map("frame"); // 创建地图实例
			...
			map.addMarker = function(lon, lat, content, title) {
				 ...
				 * BMap.Icon("http://developer.baidu.com/map/jsdemo/img/fox.gif",
				 * new BMap.Size(300,157)); var marker2 = new
				 * BMap.Marker(pt,{icon:myIcon}); // 创建标注
			...
			map.panel = function(data_info) {
				...
				for (var i = 0; i < data_info.length; i++) {
					...
					addMarker(data_info[i][0], data_info[i][1], content, title);
				}
			};
			...
			map.measure = function() {
				 ...
				 * 米。'; //获取两点距离,保留小数点后两位 var polyline = new
				 * BMap.Polyline([pointA,pointB], {strokeColor:"red",
				 * strokeWeight:3, strokeOpacity:0.5}); //定义折线
			...
			map.marker = function() {
				map.addEventListener("click", function(e) {
					...
					var myIcon = new BMap.Icon("images/Marker.png",
							new BMap.Size(26, 26));
